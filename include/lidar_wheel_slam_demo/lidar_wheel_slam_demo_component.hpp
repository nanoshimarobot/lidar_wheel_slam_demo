...
//   bool approx_zero(const double val, const double epsilon = 1e-6) { return std::fabs(val) < epsilon; }
...
//       const auto& pcl_point = pcl_cloud->points[i];
//       points[i] = Eigen::Vector4d(pcl_point.x, pcl_point.y, pcl_point.z, 1.0);
//     }
//     gtsam_points::PointCloudCPU::Ptr cloud = std::make_shared<gtsam_points::PointCloudCPU>(points);
//     return cloud;
//   }
...
//       std::bind(&LidarWheelSLAMDemo::timer_cb, this));
//   }
...
//       Eigen::Isometry3d T_world_robot = frames_[i]->T_world_robot;
//       Eigen::Quaterniond q_world_robot(T_world_robot.linear());
//       pose_stamped.pose.position.x = T_world_robot.translation().x();
//       pose_stamped.pose.position.y = T_world_robot.translation().y();
//       pose_stamped.pose.position.z = T_world_robot.translation().z();
//       pose_stamped.pose.orientation.x = q_world_robot.x();
//       pose_stamped.pose.orientation.y = q_world_robot.y();
//       pose_stamped.pose.orientation.z = q_world_robot.z();
//       pose_stamped.pose.orientation.w = q_world_robot.w();
...
//     RCLCPP_INFO(
//       this->get_logger(),
//       "Synchronized data received: PointCloud2 with %d points, Wheel odometry at time %f.",
...
//       pr_wheel_T_world_robot_.translation() = Eigen::Vector3d(odom_msg->pose.position.x, odom_msg->pose.position.y, odom_msg->pose.position.z);
//       pr_wheel_T_world_robot_.linear() =
//         Eigen::Quaterniond(odom_msg->pose.orientation.w, odom_msg->pose.orientation.x, odom_msg->pose.orientation.y, odom_msg->pose.orientation.z).toRotationMatrix();

//       gtsam::Values initial_estimate;
//       initial_estimate.insert(X(0), gtsam::Pose3());

//       gtsam::NonlinearFactorGraph graph;
//       graph.emplace_shared<gtsam::PriorFactor<gtsam::Pose3>>(X(0), gtsam::Pose3(), gtsam::noiseModel::Isotropic::Precision(6, 1e6));
...
//     cr_wheel_T_world_robot_.translation() = Eigen::Vector3d(odom_msg->pose.position.x, odom_msg->pose.position.y, odom_msg->pose.position.z);
//     cr_wheel_T_world_robot_.linear() =
//       Eigen::Quaterniond(odom_msg->pose.orientation.w, odom_msg->pose.orientation.x, odom_msg->pose.orientation.y, odom_msg->pose.orientation.z).toRotationMatrix();
...
//       RCLCPP_WARN(this->get_logger(), "Negligible wheel odometry change detected. Skipping frame.");
//       return;
//     }

//     RCLCPP_INFO(this->get_logger(), "wheel_T_cr_pr trans : %lf, angle : %lf", wheel_T_pr_cr.translation().norm(), Eigen::AngleAxisd(wheel_T_pr_cr.linear()).angle());
...
//     Eigen::Isometry3d init_T_world_robot = frames_[frame_idx - 1]->T_world_robot * wheel_T_pr_cr;
//     initial_estimate.insert(X(frame_idx), gtsam::Pose3(init_T_world_robot.matrix()));

//     graph.emplace_shared<gtsam::BetweenFactor<gtsam::Pose3>>(X(frame_idx - 1), X(frame_idx), gtsam::Pose3(wheel_T_pr_cr.matrix()),
//     gtsam::noiseModel::Isotropic::Precision(6, 1.0)); graph.emplace_shared<gtsam_points::IntegratedICPFactor>(X(frame_idx - 1), X(frame_idx), frames_[frame_idx - 1]->cloud,
//     new_frame->cloud);

//     isam2_->update(graph, initial_estimate);
//     auto estimated_values = isam2_->calculateEstimate();
//     for(gtsam::Symbol&& key : estimated_values.keys()) {
//       size_t idx = key.index();
//       gtsam::Pose3 est_pose = estimated_values.at<gtsam::Pose3>(key);
